...
eb1<-state_base+geom_polygon(data=stco,aes(fill=LifeExp),color="white") +geom_polygon(color="black",fill=NA)+coord_fixed(1.3)+theme_bw()+ggtitle("Life Expectancy in each State")
#assign color gradeint to map based on values
eb2<-eb1+scale_fill_gradient(low="lightblue", high="darkblue")
#displays map
eb2
